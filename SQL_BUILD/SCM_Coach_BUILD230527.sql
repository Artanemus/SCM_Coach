 ...
 *
 * Date Created : Saturday, May 27, 2023 12:17:54
...
CREATE TABLE PlayScriptTag(
    PlayScriptTagID    int              IDENTITY(1,1),
    Caption            nvarchar(128)    NULL,
    CONSTRAINT PK_PlayScriptTag PRIMARY KEY CLUSTERED (PlayScriptTagID)
)
go



IF OBJECT_ID('PlayScriptTag') IS NOT NULL
    PRINT '<<< CREATED TABLE PlayScriptTag >>>'
ELSE
    PRINT '<<< FAILED CREATING TABLE PlayScriptTag >>>'
go
...
CREATE TABLE Reserved(
    ReservedID    int              IDENTITY(1,1),
    Caption       nvarchar(128)    NULL,
    CONSTRAINT PK_Reserved PRIMARY KEY CLUSTERED (ReservedID)
)
go



IF OBJECT_ID('Reserved') IS NOT NULL
    PRINT '<<< CREATED TABLE Reserved >>>'
ELSE
    PRINT '<<< FAILED CREATING TABLE Reserved >>>'
go
...
ALTER TABLE evTime ADD CONSTRAINT heartRangeevT22 
...
ALTER TABLE PlayScript ADD CONSTRAINT PlayScriptTagPlayScript 
    FOREIGN KEY (PlayScriptTagID)
    REFERENCES PlayScriptTag(PlayScriptTagID)
go
...
ALTER TABLE Task ADD CONSTRAINT ReservedTask 
    FOREIGN KEY (ReservedID)
    REFERENCES Reserved(ReservedID)
go
